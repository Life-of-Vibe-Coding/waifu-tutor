...
const moodColor: Record<CharacterMood, string> = {
  neutral: "from-white/70 to-aqua/55",
  happy: "from-sakura/75 to-bubblegum/55",
  encouraging: "from-butter/70 to-cream/55",
  sad: "from-lavender/70 to-pink/55",
  excited: "from-sakura/80 to-lavender/65",
};
    ...
    <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-end justify-center">
        <div className="relative h-[80vh] w-[min(92vw,840px)]">
          ...
          <div
            className={`absolute right-3 top-4 rounded-full border border-white/75 bg-gradient-to-r px-4 py-2 text-xs font-bold uppercase tracking-[0.18em] text-slate-700 sm:right-6 sm:top-6 ${moodColor[mood]}`}
          >
            Mood: {moodLabel[mood]}
          </div>
          <div className="absolute left-3 top-4 rounded-full border border-white/75 bg-white/35 px-4 py-2 text-xs font-bold uppercase tracking-[0.18em] text-slate-700 sm:left-6 sm:top-6">
            Status: {statusLabel[companionStatus]}
          </div>
        </div>
      </div>
    </div>
