...
export const NotesPage = () => {
  ...
  const foldersQuery = useQuery({ queryKey: ["noteFolders"], queryFn: listNoteFolders, retry: false });
  const folders = foldersQuery.data ?? [];
  const notesUnavailable = foldersQuery.isError;
    ...
    return (
      <section className="flex h-full flex-col overflow-hidden">
        <header className="flex shrink-0 items-center gap-3 border-b border-white/50 bg-white/30 px-4 py-3 backdrop-blur-sm">
          <h1 className="font-display text-xl font-bold tracking-tight text-ink sm:text-2xl">学习笔记</h1>
        </header>
        <div className="flex flex-1 flex-col items-center justify-center gap-3 text-slate-600">
          <p className="text-sm font-medium">Notes are not yet available in this version.</p>
          <p className="text-xs">The notes API will be migrated in a future update.</p>
        </div>
      </section>
    );
  }
