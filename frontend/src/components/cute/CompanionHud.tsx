...
export const CompanionHud = ({ className }: Props) => {
  ...
  const resetTimeoutRef = useRef<number | ReturnType<typeof setTimeout> | null>(null);
  const filledHearts = useMemo(() => Math.round((affection / 100) * 5), [affection]);

  const triggerSparkle = () => setBurst((v) => v + 1);
  const scheduleIdle = () => {
    if (resetTimeoutRef.current) window.clearTimeout(resetTimeoutRef.current);
    resetTimeoutRef.current = window.setTimeout(() => setCompanionStatus("idle"), 1000);
  };
  ...
  return (
    <motion.aside
      className={`relative w-[15.5rem] rounded-2xl border border-white/70 bg-white/40 p-3 shadow-glowPink backdrop-blur-md ${className}`}
      aria-label="Companion status and actions"
      initial={{ opacity: 0, y: -14, scale: 0.96 }}
      animate={{ opacity: 1, y: 0, scale: 1 }}
      transition={{ duration: 0.36, ease: "easeOut" }}
    >
      <motion.div
        className={`rounded-full border border-white/80 bg-gradient-to-r px-3 py-1.5 text-xs font-bold uppercase tracking-wider ${statusTone[status]}`}
        animate={{
          scale: status === "celebrating" ? [1, 1.05, 1] : 1,
          boxShadow: status === "celebrating" ? ["0 0 0 rgba(0,0,0,0)", "0 0 22px rgba(255, 142, 197, 0.46)", "0 0 0 rgba(0,0,0,0)"] : "0 0 0 rgba(0,0,0,0)",
        }}
        transition={{ duration: 1.1, repeat: status === "celebrating" ? Infinity : 0 }}
      >
        {statusLabel[status]}
      </motion.div>
      <div className="mt-3 rounded-xl border border-white/70 bg-white/50 p-3 shadow-sticker">
        <p className="text-xs font-bold uppercase tracking-wider text-slate-600">Affection</p>
        <div className="mt-1.5 flex items-center gap-1.5 text-lg text-bubblegum">
          {Array.from({ length: 5 }, (_, i) => (
            <span key={i} className={i < filledHearts ? "tiny-bounce" : "opacity-35"} aria-hidden>❤</span>
          ))}
          <span className="ml-1 text-sm font-bold text-slate-700">{affection}%</span>
        </div>
        <p className="mt-3 text-xs font-bold uppercase tracking-wider text-slate-600">Session Streak</p>
        <div className="mt-1 flex items-center gap-2">
          <span className="rounded-full bg-butter/70 px-2 py-0.5 text-xs font-extrabold text-amber-800">{sessionStreak}</span>
      ...
      <div key={burst} className="pointer-events-none absolute inset-0 overflow-hidden rounded-2xl">
        {Array.from({ length: 6 }, (_, i) => (
          <motion.span
            ...
            animate={{
              x: [0, Math.cos((i / 6) * Math.PI * 2) * 42],
              y: [0, Math.sin((i / 6) * Math.PI * 2) * 28 - 8],
              opacity: [0, 1, 0],
              scale: [0.4, 1.2, 0.85],
            }}
