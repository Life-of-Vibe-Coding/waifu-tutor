...
export const GmailConnect = ({ onClose }: GmailConnectProps) => {
  ...
  const handleConnect = async () => {
    try {
      ...
      // Not configured or 501
    }
  };

  const is501 = statusQuery.error && (statusQuery.error as { response?: { status?: number } })?.response?.status === 501;
  const connected = statusQuery.data?.connected ?? false;
  const configured = !statusQuery.isError && !is501 && (statusQuery.data?.configured ?? false);
    ...
    >
      ...
      <div className="mt-3">
        <AnimatePresence mode="wait">
          {is501 || statusQuery.isError ? (
            <p className="text-xs text-slate-600">
              Gmail integration is not yet available in this version.
            </p>
          ) : !configured ? (
            <p className="text-xs text-slate-600">
              ...
              <code className="rounded bg-slate-200/80 px-1">GMAIL_REDIRECT_URI</code> in backend .env
            </p>
          ) : connected ? (
            <motion.div
              key="connected"
