...
def _run_chat(body: ChatBody) -> dict[str, Any]:
    ...
    text = ai_chat(msg, context_texts, attachment_title, conversation_history=history, system_persona=persona)
...
def chat_stream(body: ChatBody) -> StreamingResponse:
    payload = _run_chat(body)
    text = str((payload.get("message") or {}).get("content") or "")
    mood = str(payload.get("mood") or "neutral")
    context = payload.get("context") or []
    session_id = payload.get("session_id")
    ov_error = payload.get("openviking_error")
    search_trace = payload.get("search_trace")

    def event_stream():
        context_payload: dict[str, Any] = {"context": context, "session_id": session_id}
        if ov_error:
            context_payload["openviking_error"] = ov_error
        ...
        done_payload: dict[str, Any] = {"message": text, "session_id": session_id}
        if ov_error:
            done_payload["openviking_error"] = ov_error
